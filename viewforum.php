 ...
 *
 *   $Id: viewforum.php,v 1.9 2004/12/03 23:51:42 blackdeath_csmc Exp $
 *
 *
 ***************************************************************************/
...
/*$sql = "SELECT u.uid, u.uname
    ...
    WHERE aa.forum_id = $forum_id
        ...
        AND ug.groupid = aa.groupid
        AND g.groupid = aa.groupid
        AND u.uid = ug.uid
...
{
    $moderators[] = '<a href="' . append_sid("profile.php?mode=viewprofile&amp;" . POST_USERS_URL . "=" . $row['uid']) . '">' . $row['uname'] . '</a>';
}*/

$sql = 'SELECT g.groupid, g.name AS group_name
...
while (false !== ($row = $db->sql_fetchrow($result))) {
    $moderators[] = '<a href="' . append_sid('groupcp.php?' . POST_GROUPS_URL . '=' . $row['groupid']) . '">' . $row['group_name'] . '</a>';
}
...
if ($is_auth['auth_mod']) {
    $s_auth_can .= sprintf($lang['Rules_moderate'], '<a href="modcp.php?' . POST_FORUM_URL . "=$forum_id&amp;start=" . $start . '&amp;sid=' . $userdata['sess_id'] . '">', '</a>');
}
...
if ($total_topics) {
    for ($i = 0; $i < $total_topics; $i++) {
        ...
        $topic_author = (ANONYMOUS != $topic_rowset[$i]['uid']) ? '<a href="' . append_sid('profile.php?mode=viewprofile&amp;' . POST_USERS_URL . '=' . $topic_rowset[$i]['uid']) . '">' : '';
        ...
        $last_post_author = (ANONYMOUS == $topic_rowset[$i]['id2']) ? (('' != $topic_rowset[$i]['post_username2']) ? $topic_rowset[$i]['post_username2'] . ' ' : $lang['Guest'] . ' ') : '<a href="'
                                                                                                                                                                                         . append_sid('profile.php?mode=viewprofile&amp;' . POST_USERS_URL . '=' . $topic_rowset[$i]['id2'])
                                                                                                                                                                                         . '">'
                                                                                                                                                                                         . $topic_rowset[$i]['user2']
                                                                                                                                                                                         . '</a>';

        $last_post_url = '<a href="'
                         . append_sid('viewtopic.php?' . POST_POST_URL . '=' . $topic_rowset[$i]['topic_last_post_id'])
                         . '#'
