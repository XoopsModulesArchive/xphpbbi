 ...
 *
 *   $Id: admin_board.php,v 1.7 2004/11/30 21:54:46 blackdeath_csmc Exp $
 *
 *
 ***************************************************************************/
    ...
    while (false !== ($row = $db->sql_fetchrow($result))) {
        ...
        if (isset($_POST['submit'])) {
            $sql = 'UPDATE ' . CONFIG_TABLE . " SET
				...
				WHERE config_name = '$config_name' AND config_name!='gzip_compress'";

            // Koudanshi hack Gzip

            $db->sql_query('UPDATE ' . CONFIG_TABLE . " SET config_value=0 WHERE config_name='gzip_compress'");

            // Koudanshi hack XOOPS activation type ------------------

            if ('require_activation' == $config_name) {
                if (0 == $new[$config_name]) {
                    $activation_type = 1;
                } elseif (1 == $new[$config_name]) {
                    $activation_type = 0;
                } else {
                    $activation_type = 2;
                }

                if (!$db->sql_query('UPDATE ' . XOOPS_DB_PREFIX . "_config SET conf_value=$activation_type WHERE conf_name='activation_type'")) {
                    echo "UPDATE '.XOOPS_DB_PREFIX.'_config SET conf_value='.$activation_type.' WHERE conf_name='activation_type'";

                    die('Can not update XOOPS config table Activation Type');
                }
            }
...
$template->assign_vars(
    [
        ...
        'L_ENABLE_GZIP' => $lang['Enable_gzip'] . "<br><font color=red><b>Please don't use this function!.</b></font>",
