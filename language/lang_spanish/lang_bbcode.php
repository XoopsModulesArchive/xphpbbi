...
$faq[] = ['--', 'Introducción'];
$faq[] = ['¿Qué es el código BBCode?', 'BBCode es una implementación especial del HTML, la forma en la que BBCode se usa es determinada por el administrador, es muy similar al HTML, las etiquetas van entre corchetes [ y ]'];

$faq[] = ['--', 'Formateo de texto'];
$faq[] = ['¿Cómo crear texto en negritas, cursiva o subrayado?', 'BBCode incluye etiquetas para esto: [b][/b] para negritas, [u][/u] para subrayar y [i][/i] para cursivas, estas se pueden combinar entre si, es genial! :)'];
$faq[] = [
    '¿Cómo cambiar el color o tamaño de texto?',
    'Para cambiar el color: [color=][/color], puede escribir el nombre del color en inglés o el código hexadecimal perteneciente a él, ej. #FFFFFF, #000000.  para crear rojo [color=red]Hola![/color]. Cambiar el tamaño es similar: [size=][/size], utilizando números del 1 al 29 (muy grande!)',
];
$faq[] = ['¿Puedo combinar las etiquetas de formato?', 'Si :)'];

$faq[] = ['--', 'Haciendo citas de texto o código'];
$faq[] = [
...
$faq[] = ['Escribiendo código o texto de otro tamaño', 'Al escribir código será puesto en una fuente tipo Typewriter, como Courier, solo encierre el texto entre las etiquetas [code][/code] de esta forma: [code]echo "Esto se supone es código";[/code].'];
...
$faq[] = [
    ...
    'El segundo tipo de lista es la ordenada, para crearla use [list=1][/list] para crear una lista con numeración o [list=a][/list] para una con orden alfabético, cada parte de la lista se especifica tambien con [*] Por ejemploe: [list=1][*]Vaca[*]Cuyo[*]Conejo[/list] generará algo como: <ol><li>Vaca</li><li>Cuyo</li><li>conejo</li></ol>',
];

$faq[] = ['--', 'Creando Enlaces'];
$faq[] = [
    'Creando un enlace a otro sitio',
    'phpBB BBCode soporta varias formas de hacer un enlace, la primera es con [url=][/url], por ejemplo, para hacer un enlace a phpBB.com puede usar:[url=http://www.phpbb.com/]Visite phpBB![/url], los enlaces se abrirán en una nueva ventana nueva, otra forma es [url]http://www.phpbb.com/[/url]. Este foro tiene tambien ENLACES MÁGICOS, por ejemplo, si teclea www.phpbb.com en su mensaje aparecerá automaticamente como enlace. Para hacer un enlace a un correo electrónico deberá poner: [email]alguien@sudireccion.com[/email] o simplemente teclear la direccion y se convertirá en un enlace. Puede combinarlo con la etiqueta [img][/img] para que el enlace sea una imagen, así: [url=http://www.phpbb.com/][img]http://www.phpbb.com/images/phplogo.gif[/url][/img].',
];

$faq[] = ['--', 'Publicando imágenes en los mensajes'];
$faq[] = [
    'Agregando una imagen al mensaje',
    'Para poner una imagen simplemente escriba [img]URL[/img] donde URL es la dirección en donde está su imagen, por ejemplo [img]http://www.phpbb.com/images/phplogo.gif[/img], tambien puede generar enlaces de la siguiente forma: [url=][/url] así [url=http://www.phpbb.com/][img]http://www.phpbb.com/images/phplogo.gif[/img][/url]',
];

$faq[] = ['--', 'Otros'];
$faq[] = ['¿Puedo agregar mis propias etiquetas?', 'No, no en esta version de phpBB (2), seguramente será en versiones posteriores a esta'];
