 ...
 *
 *   $Id: auth.php,v 1.9 2004/12/04 19:39:30 blackdeath_csmc Exp $
 *
 *
 ***************************************************************************/
...
function auth($type, $forum_id, $userdata, $f_access = '')
{
    ...
    if (is_object($xoopsUser)) {
        ...
        $sql = "SELECT a.forum_id, $a_sql, a.auth_mod
			...
			WHERE ug.uid = ' . $userdata['uid'] . "
				AND a.groupid = ug.groupid
				$forum_match_sql";
    ...
    $is_admin = (is_object($xoopsUser) && $xoopsUserIsAdmin) ? true : 0;
        ...
        if (AUTH_LIST_ALL != $forum_id) {
            ...
            switch ($value) {
                case AUTH_ALL:
                    ...
                    break;
                case AUTH_REG:
                    $auth_user[$key] = (is_object($xoopsUser)) ? true : 0;
                    $auth_user[$key . '_type'] = $lang['Auth_Registered_Users'];
                    break;
                case AUTH_ACL:
                    $auth_user[$key] = (is_object($xoopsUser)) ? auth_check_user(AUTH_ACL, $key, $u_access, $is_admin) : 0;
                    $auth_user[$key . '_type'] = $lang['Auth_Users_granted_access'];
                    break;
                case AUTH_MOD:
                    $auth_user[$key] = (is_object($xoopsUser)) ? auth_check_user(AUTH_MOD, 'auth_mod', $u_access, $is_admin) : 0;
            ...
            for ($k = 0, $kMax = count($f_access); $k < $kMax; $k++) {
                ...
                switch ($value) {
                    case AUTH_ALL:
                        ...
                        break;
                    case AUTH_REG:
                        $auth_user[$f_forum_id][$key] = (is_object($xoopsUser)) ? true : 0;
                        $auth_user[$f_forum_id][$key . '_type'] = $lang['Auth_Registered_Users'];
                        break;
                    case AUTH_ACL:
                        $auth_user[$f_forum_id][$key] = (is_object($xoopsUser)) ? auth_check_user(AUTH_ACL, $key, $u_access[$f_forum_id], $is_admin) : 0;
                        $auth_user[$f_forum_id][$key . '_type'] = $lang['Auth_Users_granted_access'];
                        break;
                    case AUTH_MOD:
                        $auth_user[$f_forum_id][$key] = (is_object($xoopsUser)) ? auth_check_user(AUTH_MOD, 'auth_mod', $u_access[$f_forum_id], $is_admin) : 0;
    ...
    if (AUTH_LIST_ALL != $forum_id) {
        $auth_user['auth_mod'] = (is_object($xoopsUser)) ? auth_check_user(AUTH_MOD, 'auth_mod', $u_access, $is_admin) : 0;
    } else {
        for ($k = 0, $kMax = count($f_access); $k < $kMax; $k++) {
            $f_forum_id = $f_access[$k]['forum_id'];

            $auth_user[$f_forum_id]['auth_mod'] = (is_object($xoopsUser)) ? auth_check_user(AUTH_MOD, 'auth_mod', $u_access[$f_forum_id], $is_admin) : 0;
        }
    }
