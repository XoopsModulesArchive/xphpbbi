 ...
 *
 *   $Id: page_header.php,v 1.10 2004/12/03 23:51:42 blackdeath_csmc Exp $
 *
 *
 ***************************************************************************/
...
$xoopsTpl->assign(
    'xoops_module_header',
    '
<link rel="stylesheet" type="text/css" href="' . XOOPS_URL . '/modules/xphpbbi/templates/' . $theme['template_name'] . '/' . $theme['head_stylesheet'] . '">
...
$s_last_visit = (is_object($xoopsUser)) ? create_date($board_config['default_dateformat'], $userdata['user_lastvisit'], $board_config['board_timezone']) : '';

//
// Fetch moderator groups
//
$mod_array = get_mod_groups();
...
if (defined('SHOW_ONLINE')) {
    $user_forum_sql = (!empty($forum_id)) ? 'AND se.session_page = ' . (int)$forum_id : '';

    $sql = 'SELECT u.uname, u.uid, ue.user_allow_viewonline, ue.user_level, se.session_logged_in, s.sess_ip
		FROM ' . USERS_TABLE . ' u, ' . SESSIONS_TABLE . ' s
		LEFT JOIN ' . USERS_TABLE_EXT . ' ue ON u.uid=ue.uid
		LEFT JOIN ' . SESSIONS_TABLE_EXT . ' se ON s.sess_id=se.sess_id
		WHERE u.uid = se.session_user_id
    ...
    while (false !== ($row = $db->sql_fetchrow($result))) {
        ...
        if ($row['session_logged_in']) {
            ...
            if ($row['uid'] != $prev_user_id) {
                ...
                } elseif (array_intersect($thisUser->getGroups(), $mod_array)) { // detect mod
                    $row['uname'] = '<b>' . $row['uname'] . '</b>';
                ...
                if ($row['user_allow_viewonline']) {
                    $user_online_link = '<a href="' . append_sid('profile.php?mode=viewprofile&amp;' . POST_USERS_URL . '=' . $row['uid']) . '"' . $style_color . '>' . $row['uname'] . '</a>';

                    $logged_visible_online++;
                } else {
                    $user_online_link = '<a href="' . append_sid('profile.php?mode=viewprofile&amp;' . POST_USERS_URL . '=' . $row['uid']) . '"' . $style_color . '><i>' . $row['uname'] . '</i></a>';

                    $logged_hidden_online++;
                }

                if ($row['user_allow_viewonline'] || $xoopsUserIsAdmin) {
...
//
if ((is_object($xoopsUser)) && (empty($gen_simple_header))) {
